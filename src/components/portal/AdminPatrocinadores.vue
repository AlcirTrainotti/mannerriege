...
import { tipoPatrocinadorOptions, nivelPatrocinioOptions } from '../../data/financeiro.js'
...
const novoNome = ref('')
const novoTipo = ref('privado')
const novoNivel = ref('Basic')
...
async function carregar() {
  loading.value = true
  loadError.value = ''
  const { data, error } = await supabase.from('patrocinadores').select('*').order('nome')
  if (error) {
...
function limparForm() {
  novoNome.value = ''
  novoTipo.value = 'privado'
  novoNivel.value = 'Basic'
  novoValor.value = 0
...
async function adicionar() {
  ...
  const { error } = await supabase.from('patrocinadores').insert({
    ...
    tipo: novoTipo.value,
    nivel: novoNivel.value || null,
    valor_anual: novoValor.value || null,
...
async function salvarCampo(item, campo, valor) {
  ...
  const { error } = await supabase.from('patrocinadores').update({ [campo]: valor }).eq('id', item.id)
  savingId.value = null
  if (error) {
...
async function carregarNegociacoes(id) {
  ...
  const { data } = await supabase.from('patrocinador_negociacoes').select('*').eq('patrocinador_id', id).order('data', { ascending: false })
...
async function adicionarNegociacao(id) {
  ...
  await supabase.from('patrocinador_negociacoes').insert({
    patrocinador_id: id,
...
async function removerNegociacao(negId, patrocinadorId) {
  await supabase.from('patrocinador_negociacoes').delete().eq('id', negId)
  negociacoesPorItem.value[patrocinadorId] = (negociacoesPorItem.value[patrocinadorId] ?? []).filter((n) => n.id !== negId)
}
...
<template>
  <div>
    <div class="flex flex-wrap items-center gap-3">
      ...
      <button class="flex-shrink-0 rounded-full bg-brand px-5 py-2.5 font-mono-label text-[11px] font-bold text-white hover:bg-brand-deep" @click="showModal = true">+ Novo patrocinador</button>
    </div>
    ...
    <div v-else class="mt-5 space-y-3">
      <div v-if="filtrados.length === 0" class="py-8 text-center text-sm text-ink-soft">Nenhum patrocinador cadastrado ainda.</div>
      ...
      <div v-for="i in paginados" :key="i.id" class="rounded-2xl bg-white p-5 shadow-card">
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
          <div>
            ...
            <select :value="i.tipo" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-2 py-2 text-xs text-ink outline-none focus:border-brand" @change="(e) => { i.tipo = e.target.value; salvarCampo(i, 'tipo', e.target.value) }">
              <option v-for="t in tipoPatrocinadorOptions" :key="t.value" :value="t.value">{{ t.label }}</option>
            </select>
          </div>
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Nível</label>
            <select :value="i.nivel" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-2 py-2 text-xs text-ink outline-none focus:border-brand" @change="(e) => { i.nivel = e.target.value; salvarCampo(i, 'nivel', e.target.value) }">
              <option v-for="n in nivelPatrocinioOptions" :key="n" :value="n">{{ n }}</option>
            </select>
          </div>
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Valor anual</label>
            <CurrencyInput :model-value="i.valor_anual" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" @update:model-value="(v) => salvarCampo(i, 'valor_anual', v)" />
          </div>
          <div>
        ...
        <div class="mt-3">
          ...
          <div v-if="historicoAberto === i.id" class="mt-2 rounded-xl bg-paper-dim p-3">
            <div class="flex flex-wrap items-center gap-2">
              <input v-model="novaNegociacaoDescricao" type="text" placeholder="Ex: Renovação combinada por telefone" class="min-w-0 flex-1 rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" />
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 px-4" @click.self="showModal = false; limparForm()">
      <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-card">
        <h2 class="font-display text-2xl font-extrabold text-ink">Novo patrocinador</h2>
        <div class="mt-6 space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              <label class="font-mono-label text-[9px] font-bold text-ink-soft">Nível</label>
              <select v-model="novoNivel" class="mt-1 w-full rounded-xl border border-ink/15 bg-white px-4 py-2.5 text-sm text-ink outline-none focus:border-brand">
                <option v-for="n in nivelPatrocinioOptions" :key="n" :value="n">{{ n }}</option>
              </select>
            </div>
          </div>
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Valor anual</label>
            <CurrencyInput v-model="novoValor" class="mt-1 w-full rounded-xl border border-ink/15 bg-white px-4 py-2.5 text-sm text-ink outline-none focus:border-brand" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="font-mono-label text-[9px] font-bold text-ink-soft">Contato</label>
