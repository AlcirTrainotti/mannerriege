...
async function carregar() {
  loading.value = true
  loadError.value = ''
  const { data, error } = await supabase.from('locais_festa').select('*').order('nome')
  if (error) {
...
async function adicionar() {
  ...
  const { error } = await supabase.from('locais_festa').insert({
    nome: novoNome.value.trim(),
    endereco: novoEndereco.value.trim() || null,
    capacidade: novaCapacidade.value ? Number(novaCapacidade.value) : null,
    valor_locacao: novoValor.value || null,
...
async function salvarCampo(item, campo, valor) {
  ...
  const { error } = await supabase.from('locais_festa').update({ [campo]: valor }).eq('id', item.id)
  savingId.value = null
  if (error) {
...
async function remover(id) {
  ...
  const { error } = await supabase.from('locais_festa').delete().eq('id', id)
...
async function carregarNegociacoes(id) {
  ...
  const { data } = await supabase.from('local_festa_negociacoes').select('*').eq('local_festa_id', id).order('data', { ascending: false })
...
async function adicionarNegociacao(id) {
  ...
  await supabase.from('local_festa_negociacoes').insert({
    local_festa_id: id,
...
async function removerNegociacao(negId, localId) {
  await supabase.from('local_festa_negociacoes').delete().eq('id', negId)
  negociacoesPorItem.value[localId] = (negociacoesPorItem.value[localId] ?? []).filter((n) => n.id !== negId)
}
...
<template>
  <div>
    <div class="flex flex-wrap items-center gap-3">
      ...
      <button class="flex-shrink-0 rounded-full bg-brand px-5 py-2.5 font-mono-label text-[11px] font-bold text-white hover:bg-brand-deep" @click="showModal = true">+ Novo local</button>
    </div>
    ...
    <div v-else class="mt-5 space-y-3">
      <div v-if="filtrados.length === 0" class="py-8 text-center text-sm text-ink-soft">Nenhum local cadastrado ainda.</div>
      ...
      <div v-for="i in paginados" :key="i.id" class="rounded-2xl bg-white p-5 shadow-card">
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <div class="sm:col-span-2">
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Endereço</label>
            <input :value="i.endereco" type="text" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" @change="(e) => { i.endereco = e.target.value; salvarCampo(i, 'endereco', e.target.value) }" />
          </div>
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Capacidade</label>
            <input :value="i.capacidade" type="number" min="0" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" @change="(e) => { i.capacidade = e.target.value; salvarCampo(i, 'capacidade', Number(e.target.value) || null) }" />
          </div>
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Valor da locação</label>
            <CurrencyInput :model-value="i.valor_locacao" class="mt-1 w-full rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" @update:model-value="(v) => salvarCampo(i, 'valor_locacao', v)" />
          </div>
          <div>
        ...
        <div class="mt-3">
          ...
          <div v-if="historicoAberto === i.id" class="mt-2 rounded-xl bg-paper-dim p-3">
            <div class="flex flex-wrap items-center gap-2">
              <input v-model="novaNegociacaoDescricao" type="text" placeholder="Ex: Usamos para a costelada, atendimento ótimo" class="min-w-0 flex-1 rounded-lg border border-ink/15 bg-white px-3 py-2 text-xs text-ink outline-none focus:border-brand" />
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 px-4" @click.self="showModal = false; limparForm()">
      <div class="w-full max-w-md rounded-2xl bg-white p-8 shadow-card">
        <h2 class="font-display text-2xl font-extrabold text-ink">Novo local</h2>
        <div class="mt-6 space-y-3">
          ...
          <div>
            <label class="font-mono-label text-[9px] font-bold text-ink-soft">Endereço</label>
            <input v-model="novoEndereco" type="text" class="mt-1 w-full rounded-xl border border-ink/15 bg-white px-4 py-2.5 text-sm text-ink outline-none focus:border-brand" />
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="font-mono-label text-[9px] font-bold text-ink-soft">Capacidade</label>
              <input v-model="novaCapacidade" type="number" min="0" class="mt-1 w-full rounded-xl border border-ink/15 bg-white px-3 py-2.5 text-sm text-ink outline-none focus:border-brand" />
            </div>
            <div>
              <label class="font-mono-label text-[9px] font-bold text-ink-soft">Valor da locação</label>
              <CurrencyInput v-model="novoValor" class="mt-1 w-full rounded-xl border border-ink/15 bg-white px-3 py-2.5 text-sm text-ink outline-none focus:border-brand" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <div>
