...
async function marcarVisualizados(lista) {
  ...
  await supabase.from('avisos_visualizacoes').upsert(linhas, { onConflict: 'aviso_id,profile_id', ignoreDuplicates: true })
}
  ...
  <div class="mx-auto max-w-lg">
    <div class="flex items-center justify-between">
      <div>
        ...
        <h1 class="mt-1 font-display text-3xl font-extrabold text-ink">Olá, {{ profile?.nome }}</h1>
      </div>
      <button class="rounded-full border border-ink/15 px-4 py-2 text-xs font-semibold text-ink-soft hover:border-ink/30" @click="logout">
        Sair
      </button>
    </div>
