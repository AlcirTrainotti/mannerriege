# Como configurar o e-mail do formulário "Fale conosco"

O site institucional (site em Astro, pasta `apps/site`) tem uma página
`/contato` com o formulário de "Fale conosco". Quando alguém preenche e
envia, um e-mail precisa chegar para a diretoria com a mensagem. Isso é
feito por uma Edge Function do Supabase (`fale-conosco`), que usa o
serviço **Resend** (https://resend.com) para enviar o e-mail — o
Supabase sozinho não envia e-mail arbitrário, só usa e-mail para os
fluxos de login.
...
Se você já configurou o `RESEND_API_KEY` para os outros formulários do
site (`interesse-associado`, `matricula-base`, `interesse-apoio`), o
...
3. Dê o nome exato: `fale-conosco`
4. Apague o conteúdo de exemplo.
5. Abra o arquivo `supabase/functions/fale-conosco/index.ts` (dentro da
   pasta de código deste projeto), copie todo o conteúdo e cole no
   editor do Supabase.
6. Clique em **Deploy**.
...
código (`DESTINATARIOS` em `fale-conosco/index.ts`):
...
1. No site publicado (ou rodando `npm run dev` em `apps/site`), abra
   `/contato` e preencha o formulário com dados de teste.
2. Envie e confira se o e-mail chega nas duas caixas de entrada
   (verifique a caixa de spam na primeira vez).

Se algo não funcionar, volte em Edge Functions → `fale-conosco` →
